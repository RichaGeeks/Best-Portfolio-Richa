...
function ExperienceBook() {
  ...
  const isTimelineActive = (pageNum) => currentPage === pageNum;
  ...
  return (
    <div
      className="relative flex flex-col items-center w-full max-w-screen-xl mx-auto px-4 py-20"
      style={{ cursor }}
    ...
    >
      ...
      <div
        className={`absolute left-[15%] top-[20%] max-w-[380px] text-white pointer-events-none transition-all duration-700 ease-in-out hidden md:block ${
      ...
      {/* ✅ FIXED WRAPPER - NO SCROLL BAR */}
      <div className="w-full overflow-hidden px-2 flex justify-center">
        <HTMLFlipBook
          ref={bookRef}
          width={400}
          height={480}
          ...
          showCover={true}
          style={{ borderRadius: '3rem', cursor }}
          onFlip={handleFlip}
          className=""
        >
          {/* Cover Page */}
      ...
      <div className="w-full max-w-5xl hidden md:block mt-5 mx-auto px-4">
        <div className="relative">
          <div className="bg-gradient-to-br from-[#212121] to-[#121212] rounded-2xl p-6 pt-2 shadow-2xl border border-gray-600">
            <div className="flex justify-between items-center relative px-0">
              ...
              {Object.entries(timelinePages).map(([item, pageNum]) => {
                const isActive = isTimelineActive(pageNum);
                return (
                  <div key={item} className="flex flex-col items-center relative z-10" onClick={() => navigateToPage(pageNum)}>
                    <div
                      className={`relative w-4 h-4 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 transform hover:scale-110 ${
                        isActive
                    ...
                    >
                      ...
                      {isActive ? (
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-white relative z-10">
                    ...
                    {isActive && <div className="absolute -bottom-2 w-1 h-8 bg-gradient-to-b from-blue-500 to-transparent rounded-full"></div>}
                  </div>
                );
              })}
            </div>
